...
namespace ElectronicShop.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<ActionResult<APIResponse<userDTO>>> userchangepassword([FromQuery] UpdatedUserDTO updateduser)
        {
            try
            {
                var response = await _products.userchangepassword(updateduser);
                if (response != null) return Ok(response);
                return NotFound(response);
            }
            catch (Exception ex)
