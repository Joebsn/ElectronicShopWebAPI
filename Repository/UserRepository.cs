...
namespace ElectronicShop.Repository
{
    public class UserRepository : IUserShop
    {
                        ...
                        if (quantitytobuy <= producttobuy.numberofproducts && quantitytobuy != 0)
                        {
                            if (currentuser.balance >= price)
                            {
                                if (firstbuy == true)
                                {
                                    ...
                                    List<int> AllCurrentUserOrderID = (from c in _context.orders where c.userID == currentuser.userID
        ...
        public async Task<APIResponse<userDTO>> userlogin([FromQuery] getuserDTO getuser)
        {
            ...
            try
            {
                userresponse.Data = (from c in _context.users where (c.firstname!.ToLower() == getuser.firstname!.ToLower() && c.lastname!.ToLower() == getuser.lastname!.ToLower()
                                        && c.password!.ToLower() == getuser.password!.ToLower()) select _mapper.Map<userDTO>(c)).FirstOrDefault()!;
        ...
        public async Task<APIResponse<userDTO>> usersignup([FromQuery] CreatedUser createduser)
        {
            ...
            try
            {
                ...
                userresponse.Data = (from c in _context.users where (c.firstname!.ToLower() == createduser.firstname!.ToLower() && c.lastname!.ToLower() == createduser.lastname!.ToLower()
                                        && c.age == createduser.age && c.phonenumber == createduser.phonenumber) select _mapper.Map<userDTO>(c)).FirstOrDefault()!;
        ...
        public async Task<APIResponse<userDTO>> userchangepassword([FromQuery] UpdatedUserDTO updateduser)
        {
            APIResponse<userDTO> userresponse = new APIResponse<userDTO>();
            try
            {
                usermodel u = (from c in _context.users  where (c.firstname!.ToLower() == updateduser.firstname!.ToLower() && c.lastname!.ToLower() == updateduser.lastname!.ToLower()
                                 && c.password!.ToLower() == updateduser.password!.ToLower() && c.phonenumber == updateduser.phonenumber) select c).FirstOrDefault()!;

                if (u != null)
                {
                    u.password = updateduser.newpassword!;
            ...
            {
                userresponse.OperationFailed(ex);
                _logger.LogError(ex, s + nameof(userchangepassword));
                return userresponse;
            }
        }
    }
